#!/usr/bin/env python3
"""
JIRA MCP Server - Финальная версия с основными функциями управления задачами
"""
...
# ---------------- FastAPI App ---------------- #
app = FastAPI(title="JIRA MCP Server", version="2.8.0")
...
def format_issues_list(issues_data: dict) -> str:
    """Форматирование списка задач для вывода"""
    ...
    result = f"📋 Найдено задач: {total}\n"
    if len(issues) < total:
        result += f"Показано: {len(issues)} из {total}\n"
    result += "\n"
    ...
    for issue in issues:
        ...
        assignee_name = assignee.get('displayName') if assignee else 'Не назначен'
        priority = fields.get('priority', {}).get('name', 'Не указан')
        issue_type = fields.get('issuetype', {}).get('name', 'Неизвестно')
        due_date = fields.get('duedate', 'Не указан')
        
        # Создаем ссылку
        issue_url = f"{config.base_url}/browse/{key}"
        
        result += f"🎯 **{key}** - {summary}\n"
        result += f"   📊 Статус: {status} | 👤 Исполнитель: {assignee_name}\n"
        result += f"   ⚡ Приоритет: {priority} | 🔧 Тип: {issue_type}\n"
        if due_date != 'Не указан':
            result += f"   📅 Срок: {due_date}\n"
        result += f"   🔗 Ссылка: {issue_url}\n\n"
    
    return result

# ---------------- JIRA Methods (Core Functions) ---------------- #
...
# ---------------- Основная функция получения задач ---------------- #
async def get_all_issues(
    ...
    """Получение всех задач проекта с фильтрацией"""
    if not config.is_valid():
        raise HTTPException(status_code=500, detail="JIRA configuration not valid")
    
    # Строим JQL запрос
    jql_parts = [f'project = "{config.project_key}"']
    ...
    try:
        async with httpx.AsyncClient(
            ...
            url = urljoin(f"{config.base_url.rstrip('/')}/rest/api/3/", "search")
        ...
        error_msg = f"Failed to get issues: {str(e)}"
        logger.error(error_msg)
        raise HTTPException(status_code=500, detail=error_msg)

# ---------------- MCP Handler (Final Version) ---------------- #
async def handle_mcp_message(message: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        if method == "initialize":
            ...
            result = {
                ...
                "result": {
                    "tools": [
                        ...
                        {
                            "name": "get_all_issues",
                            "description": "Получить все задачи проекта с возможностью фильтрации",
                            "inputSchema": {
                                "type": "object",
                                "properties": {
                                    "max_results": {
                                    ...
                                    "assignee": {
                                        "type": "string",
                                        "description": "Фильтр по исполнителю (email/имя или 'unassigned')"
                                    },
                                    "issue_type": {
                                        "type": "string",
                                        "description": "Фильтр по типу задачи (Task, Bug, Story)"
                                    },
                                    "created_after": {
                                        "type": "string",
                                        "description": "Созданы после даты YYYY-MM-DD"
                                    }
                                },
                                "required": []
                            }
            ...
            if name == "create_jira_issue":
            ...
            elif name == "get_all_issues":
                result = await get_all_issues(
                ...
                formatted_issues = format_issues_list(result)
...
async def root():
    ...
    <html>
    <head>
        <title>JIRA MCP Server v2.8 - Final Edition</title>
        <style>
            ...
            .category { background: #e9ecef; padding: 10px; margin: 10px 0; border-radius: 4px; }
        </style>
    </head>
    <body>
        <div class="container">
            <h1 class="status">✅ JIRA MCP Server v2.8 - Final Edition</h1>
            
            <div class="feature">
                <h3>🎯 Основные возможности:</h3>
                <ul>
                    <li>✅ Создание задач с полными параметрами</li>
                    <li>✅ Поддержка Atlassian Document Format (ADF)</li>
                    <li>✅ Управление исполнителями и сроками</li>
                    <li>✅ Редактирование задач по названию</li>
                    <li>✅ Изменение статусов</li>
                    <li>✅ Поиск пользователей по email/имени</li>
                    <li>✅ Получение всех задач с фильтрацией</li>
                    <li>✅ Получение справочной информации</li>
                </ul>
            </div>
            
            <div class="category">
                <h3>📋 Доступные инструменты (10 штук):</h3>
                <div class="tool">create_jira_issue - Создание задач</div>
                <div class="tool">assign_jira_issue - Назначение по ключу</div>
                <div class="tool">assign_issue_by_summary - Назначение по названию</div>
                <div class="tool">edit_issue_by_summary - Редактирование</div>
                <div class="tool">set_due_date_by_summary - Установка срока</div>
                <div class="tool">change_status_by_summary - Изменение статуса</div>
                <div class="tool">get_jira_issue - Информация о задаче</div>
                <div class="tool">get_project_users - Список пользователей</div>
                <div class="tool">get_project_statuses - Список статусов</div>
                <div class="tool">get_all_issues - Получение всех задач</div>
            </div>
            
            <h3>💡 Примеры использования:</h3>
            <ul>
                <li><strong>Создание:</strong> "Создай задачу 'Тестирование API' с исполнителем john@company.com и сроком 2024-12-31"</li>
                <li><strong>Поиск:</strong> "Найди все задачи со статусом 'In Progress'"</li>
                <li><strong>Редактирование:</strong> "Измени название задачи 'Старое название' на 'Новое название'"</li>
                <li><strong>Статус:</strong> "Переведи задачу 'Тестирование' в статус 'Done'"</li>
                <li><strong>Фильтрация:</strong> "Покажи все неназначенные задачи типа Bug"</li>
            </ul>
            
            <h3>🌐 API Endpoints:</h3>
            <p><strong>MCP:</strong> <code>POST http://your-server:8000/mcp</code></p>
            <p><strong>Health:</strong> <code>GET http://your-server:8000/health</code></p>
            
            <h3>📈 Статистика:</h3>
            <p>• <strong>Всего инструментов:</strong> 10</p>
            <p>• <strong>Версия:</strong> 2.8.0</p>
        </div>
    </body>
    </html>
    """)
...
if __name__ == "__main__":
    ...
    logger.info("🚀 Starting JIRA MCP Server v2.8 Final Edition...")
    logger.info("📋 Available tools: 10")
    logger.info("   • Task Management: 9 tools")
    logger.info("   • Issue Retrieval: 1 tool (get_all_issues)")
